 ...
 *
 * $Id: mpeg2decoder.c,v 1.1 2004/08/01 05:07:03 lucke Exp $
 */
...
int cStreamDecoder::ParseStreamIntern(uchar *Data, int Length,
	...
	int size=Length;
		...
		case HEADER+4:		// PTS-Header length
		    ...
		    if (headerLength) { // we found optional headers (=PTS)
			validPTS=true;
			state=OPTHEADER;
			hdr_ptr=header;
		    }
...
// this few lines does the whole syncing
	...
	if (delay < -1000) {
	    ...
	    if (context->hurry_up) context->hurry_up--;
